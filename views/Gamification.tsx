...
export const Gamification: React.FC<GamificationProps> = ({ onTripClick }) => {
    ...
    const processTravelHistory = async (tripList: Trip[]) => {
        ...
        // 2. Resolve (Parallelized via Service Cache)
        const resolvedData = new Map<string, any>();
        const placeList = Array.from(placesToResolve);
        const resolvedResults = await mapWithConcurrency(placeList, resolvePlaceName, GEO_CONCURRENCY_LIMIT);
        resolvedResults.forEach((res, index) => {
            if (res) resolvedData.set(placeList[index], res);
        });
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-[100rem] mx-auto pb-12">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 h-[31.25rem]">
            <div className="xl:col-span-2 relative rounded-[2.5rem] overflow-hidden border border-gray-100 dark:border-white/5 shadow-2xl group">
                <Suspense fallback={
                    <div className="w-full h-full flex flex-col items-center justify-center bg-zinc-950/70 border border-white/5 space-y-4">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Loading Exploration Globe...</p>
                    </div>
                }>
            ...
            <div className="xl:col-span-1 bg-white dark:bg-gray-900 rounded-[2.5rem] p-8 border border-gray-100 dark:border-white/5 shadow-xl flex flex-col justify-between relative overflow-hidden">
                ...
                <div>
                    ...
                    <h1 className="text-4xl lg:text-5xl font-black text-gray-900 dark:text-white mt-2 mb-1 tracking-tight leading-none">{currentLevel.name}</h1>
                    <div className="flex items-center gap-2 mt-2">
                        <div className="px-3 py-1 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-xs font-black uppercase tracking-wider border border-amber-200 dark:border-amber-900/50">Level {currentLevel.level}</div>
                        {nextLevel && (<span className="text-xs text-gray-400 font-medium">{nextLevel.countries - visitedData.length} countries to {nextLevel.name}</span>)}
                    </div>
                </div>
                <div className="space-y-6 relative z-10">
                    ...
                    {nextLevel && (
                        <div>
                            <div className="flex justify-between items-end mb-2"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Progress</span><span className="text-xs font-bold text-amber-500">{Math.round(progressToNext)}%</span></div>
                            <div className="h-3 w-full bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-amber-400 to-orange-500 transition-all duration-1000 ease-out rounded-full relative" style={{ width: `${progressToNext}%` }} /></div>
                        </div>
                    )}
                </div>
            </div>
        ...
        <Tabs activeTab={activeTab} onChange={setActiveTab} tabs={[{ id: 'stamps', label: 'Passport Stamps', icon: <span className="material-icons-outlined">verified</span> }, { id: 'analytics', label: 'Flight Log', icon: <span className="material-icons-outlined">data_usage</span> }]} />
