...
export const Dashboard: React.FC<DashboardProps> = ({ onUserClick, onTripClick }) => {
  ...
  const handleSubmitRequest = async (tripData: Trip) => {
    ...
    // Workflow: If user checked "Traveling" (implying location != Time Off), redirect to TripDetail
  ...
  const renderDayCell = (date: Date, minHeightClass: string = 'min-h-[140px]', showDate = true, size: 'normal' | 'compact' = 'normal') => {
      ...
      const isTripDeductible = (t: Trip) => {
          ...
          if (!workspaceConfig?.workingDays.includes(dayOfWeek)) return false;
      ...
      return (
        <div key={dateKey} title={holidayNames || undefined} onMouseDown={() => onDayMouseDown(date)} onMouseEnter={() => onDayMouseEnter(date)} className={`relative ${minHeightClass} rounded-2xl border transition-all hover:border-blue-300 dark:hover:border-blue-700 hover:shadow-xl group flex flex-col overflow-hidden cursor-pointer ${cellBackground} ${borderColor}`}>
            ...
            {dayWeight > 0 && <div className="absolute top-2 left-3 z-20 pointer-events-none"><span className="text-[10px] font-black text-gray-400 dark:white/30 tracking-tight">{dayWeight}</span></div>}
  ...
  const renderUserCard = (user: User) => {
    ...
    // Separate Annual from others
    const annualEnt = entitlements.find(e => e.category === 'Annual' && userPolicies.some(p => p.entitlementId === e.id));
    const otherEnts = entitlements.filter(e => {
        if (e.id === annualEnt?.id) return false;
        if (e.category === 'Lieu') return true;
        return userPolicies.some(p => p.entitlementId === e.id);
    });

    const isPartner = user.role === 'Partner';
    const gradient = isPartner ? 'from-blue-600 to-indigo-700' : 'from-emerald-500 to-teal-700';

    // Calculate Annual Stats
    let annualTotal = 0;
    let annualUsed = 0;
    if (annualEnt) {
        annualTotal = getTotalAllowance(user.id, annualEnt.id, currentYear);
        annualUsed = calculateUsedDays(user.id, annualEnt.id, currentYear);
    }
    ...
    // Logic for status variables (activeTrip, nextTrip, currentStatus, daysUntilNext)
    const today = new Date();
    today.setHours(0,0,0,0);

    const activeTrip = trips.find(t => {
        if (!t.participants.includes(user.id) || t.status === 'Cancelled') return false;
    ...
    const daysUntilNext = nextTrip 
        ? Math.ceil((new Date(nextTrip.startDate).getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) 
        : 0;

    let currentStatus = { label: 'Available', color: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300' };
    if (activeTrip) {
        currentStatus = { label: 'On Leave', color: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300' };
    } else if (nextTrip && daysUntilNext <= 7) {
        currentStatus = { label: 'Leaving Soon', color: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' };
    }
    ...
    return (
        <Card 
            key={user.id} 
            noPadding 
            className="group cursor-pointer hover:-translate-y-1 transition-all bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white/20 dark:border-white/10 shadow-xl overflow-hidden relative min-h-[220px] h-full"
            ...
            <div className="flex flex-col h-full">
                ...
                <div className="p-6 pb-2 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center text-2xl font-black text-white shadow-lg transform group-hover:rotate-6 transition-all duration-300`}>
                                {user.name?.charAt(0) || '?'}
                            </div>
                            <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-white dark:bg-gray-900 rounded-full flex items-center justify-center">
                                <div className={`w-2.5 h-2.5 rounded-full ${activeTrip ? 'bg-amber-500' : 'bg-emerald-500'} animate-pulse ring-2 ring-white dark:ring-gray-900`} />
                            </div>
                        </div>
                        <div>
                            <h3 className="text-xl font-black text-gray-900 dark:text-white leading-tight tracking-tight">{user.name}</h3>
                            <div className="flex items-center gap-2 mt-1">
                                <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${currentStatus.color}`}>
                                    {currentStatus.label}
                                </span>
                    ...
                    <div className="hidden md:flex flex-col items-end">
                        {nextTrip ? (
                            <div className="text-right">
                                <div className="flex items-center gap-1.5 justify-end text-[10px] font-black uppercase text-blue-500 tracking-widest mb-0.5">
                                    <span className="material-icons-outlined text-xs">flight_takeoff</span>
                                    <span>Upcoming</span>
                                </div>
                                <div className="text-sm font-bold text-gray-700 dark:text-gray-200 truncate max-w-[150px]">{nextTrip.name}</div>
                                <div className="text-[10px] font-bold text-gray-400">In {daysUntilNext} Days</div>
                            </div>
                        ) : (
                            <div className="text-right opacity-50">
                                <div className="text-[10px] font-black uppercase text-gray-400 tracking-widest mb-0.5">Status</div>
                                <div className="text-sm font-bold text-gray-600 dark:text-gray-300">Ready for Duty</div>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="px-6 py-4 flex-1 flex flex-col justify-center relative z-10">
                    <div className="flex items-end gap-3 mb-2">
                        <span className="text-5xl font-black text-gray-900 dark:text-white leading-none tracking-tight">
                            {annualRemaining === Infinity ? '∞' : Math.floor(annualRemaining)}
                        </span>
                        <div className="mb-1.5">
                            <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest block">Days Left</span>
                        </div>
                    </div>
                    ...
                    <div className="w-full h-2 bg-gray-100 dark:bg-white/10 rounded-full overflow-hidden flex">
                        <div 
                            className={`h-full ${annualColor} transition-all duration-1000 ease-out`} 
                            style={{ width: `${Math.min(100, annualPercent)}%` }} 
                        />
                    </div>
                    <div className="flex justify-between mt-1.5">
                        <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">{Math.floor(annualUsed)} Used</span>
                        <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Allowance: {annualTotal === Infinity ? 'Unlimited' : annualTotal}</span>
                    </div>
                </div>
                ...
                <div className="px-6 py-4 bg-gray-50/50 dark:bg-black/20 border-t border-gray-100 dark:border-white/5 flex gap-2 flex-wrap items-center relative z-10">
                    <button 
                        onClick={(e) => { e.stopPropagation(); handleOpenRequest(); }}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white dark:bg-white/10 border border-gray-200 dark:border-white/5 shadow-sm hover:border-blue-300 transition-colors group/btn"
                    >
                        <span className="material-icons-outlined text-sm text-gray-400 group-hover/btn:text-blue-500 transition-colors">add_circle</span>
                        <span className="text-[10px] font-black uppercase tracking-wider text-gray-500 dark:text-gray-300">Book</span>
                    </button>
                    <div className="w-px h-4 bg-gray-300 dark:bg-white/10 mx-1" />
                    {otherEnts.length > 0 ? otherEnts.map(ent => {
                        const used = calculateUsedDays(user.id, ent.id, currentYear);
                        const allowance = getTotalAllowance(user.id, ent.id, currentYear);
                        const remaining = allowance === Infinity ? Infinity : Math.max(0, allowance - used);
                        const txtColor = getEntitlementTextClass(ent.color);
                        
                        return (
                            <div key={ent.id} className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-transparent border border-gray-200 dark:border-white/10" title={`${ent.name}: ${used} Used`}>
                                <div className={`w-1.5 h-1.5 rounded-full ${getProgressBarColor(ent.color)}`} />
                                <span className={`text-[9px] font-bold uppercase tracking-wider ${txtColor}`}>
                                    {allowance === Infinity ? '∞' : remaining.toFixed(0)} <span className="text-gray-400">{ent.name.substring(0, 3)}</span>
                                </span>
                            </div>
                        );
                    }) : (
                        <span className="text-[9px] font-medium text-gray-400 italic">No secondary policies</span>
                    )}
                </div>
            </div>
        </Card>
  ...
  const getBaseAllowance = (userId: string, entitlementId: string, year: number) => {
    ...
    if (ent.category === 'Lieu') {
        ...
        if (user.holidayWeekendRule === 'lieu') {
             const earned = holidays.filter(h => user.holidayConfigIds?.includes(h.configId || '') && h.isIncluded && new Date(h.date).getFullYear() === year && (h.isWeekend || new Date(h.date).getDay() === 0 || new Date(h.date).getDay() === 6)).length;
             total += earned;
        }
        return total;
    }
    ...
    return ent.isUnlimited ? Infinity : 0;
  };
         ...
         prevPolicies.forEach(prevP => {
              ...
              if ((targetsSelf && prevP.entitlementId === entId) || isTarget) {
                  ...
                  if (prevTotal !== Infinity) {
                      const prevUsed = calculateUsedDays(userId, prevP.entitlementId, prevYear);
                      carryOverAmount += Math.min(Math.max(0, prevTotal - prevUsed), prevP.carryOver.maxDays);
                  }
              }
         });
    }
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-[1600px] mx-auto pb-12">
        <div className="flex flex-col md:flex-row gap-6 overflow-x-auto pb-2">
            {users.map(user => (
                 <div key={user.id} className="flex-1 min-w-[350px] max-w-2xl">
                    {renderUserCard(user)}
                 </div>
            ))}
            
            <button className="group flex flex-row md:flex-col items-center justify-center px-4 py-6 rounded-[2.5rem] border-2 border-dashed border-gray-200 dark:border-white/10 text-gray-400 hover:text-blue-500 hover:border-blue-300 hover:bg-blue-50/50 dark:hover:bg-white/5 transition-all gap-2 min-h-[100px] md:w-24 shrink-0">
                <div className="w-10 h-10 rounded-full bg-gray-50 dark:bg-white/5 group-hover:bg-white dark:group-hover:bg-white/10 flex items-center justify-center transition-colors shadow-inner">
                    <span className="material-icons-outlined text-xl">add</span>
                </div>
                <span className="text-[10px] font-black uppercase tracking-widest hidden md:block md:[writing-mode:vertical-rl] md:rotate-180">Invite</span>
                <span className="text-xs font-black uppercase tracking-widest md:hidden">Invite Member</span>
            </button>
        </div>
        ...
        <LeaveRequestModal 
            isOpen={isRequestModalOpen} onClose={() => setIsRequestModalOpen(false)} onSubmit={handleSubmitRequest} onDelete={handleDeleteRequest}
            initialData={editingTrip} users={users} entitlements={entitlements} trips={trips} holidays={holidays} workspaceConfig={workspaceConfig}
        />
    </div>
  );
};
