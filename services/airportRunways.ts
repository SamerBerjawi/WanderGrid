...
export function generateAirportRunway(
    ...
    if (realRunways && realRunways.length > 0) {
        ...
        // Fallback for unlisted aerodrome using airport reference point and heading
        const rad = (fallbackHeading * Math.PI) / 180;
        const halfLenDeg = 0.015; // ~1.5km
        const dx = Math.sin(rad) * halfLenDeg;
        const dy = Math.cos(rad) * halfLenDeg;

        const startLng = lng - dx;
